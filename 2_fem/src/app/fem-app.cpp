...
public:
	...
	vector<Vector2d> get_handleTranslations()
	{
		...
		{
			ret.push_back(T_handles[i] - T_handles0[i]);
		}
		return ret;
	}
	...
	{
		vector<double> ret;
		for (int i = 0; i < theta_handles.size(); ++i)
		{
			ret.push_back(theta_handles[i] - theta_handles0[i]);
		}
		return ret;
	}
...
public:
	void drawNanoVG() override
	{
		...
		{
			auto push = CIRCLE_RADIUS;
			CIRCLE_RADIUS *= 3;
			NVGcolor c = GREEN;
			c.a = .7f;
			fillCircle(x_mouse, c);
			CIRCLE_RADIUS = push;
		}
	}
...
protected:
	...
	{
		double f = (I_AM_HAVING_SCREEN_ISSUES) ? .5 : 1.;
		x_mouse = _2xy(Vector2d(f * mouseState.lastMouseX, f * mouseState.lastMouseY));
		// --
		if (DRAG_T_i != -1)
		{
			T_handles[DRAG_T_i] = x_mouse;
		}
		if (DRAG_theta_i != -1)
		{
			theta_handles[DRAG_theta_i] = atan2_wrapper(from_(T_handles[DRAG_theta_i], x_mouse));
		}
	}
	...
	void keyPressed(int key, int mods) override
	{
		...
		if (key == GLFW_KEY_D)
		{
			toggle(sim.SOLVE_DYNAMICS);
		}
		if (key == GLFW_KEY_G)
		{
			toggle(sim.APPLY_GRAVITY);
		}
		if (key == GLFW_KEY_S)
		{
			toggle(SLOMO);
